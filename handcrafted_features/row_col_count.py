# =============================================================================
# compute number of non-zero pixel values in a given row or column
# =============================================================================

import numpy as np

# counts the number of non-zero values in a column
def col_count_nonzero(image_data, col_idx):
    return np.count_nonzero(image_data.reshape((16, 15))[:, col_idx])

# counts the number of non-zero values in a row
def row_count_nonzero(image_data, row_idx):
    return np.count_nonzero(image_data.reshape((16, 15))[row_idx, :])
...
if __name__ == "__main__":
    ...
    for col_idx in range(15):
        print(col_count_nonzero(train_data[0], col_idx))
        
    for row_idx in range(16):
        print(row_count_nonzero(train_data[0], row_idx))
